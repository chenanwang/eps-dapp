...
const client = createPublicClient({
  chain: mainnet,
  transport: http(process.env.EVM_RPC_ETH_MAINNET ?? 'https://eth-mainnet.g.alchemy.com/v2/demo'),
})
...
export async function GET() {
  ...
  if (addressMatch) {
    ensName = AGENT_ENS
    try {
      const [avatar, url, description, agentType, agentVersion] = await Promise.all([
    ...
    } catch { /* text records not yet set on-chain */ }
  }
